...
public class Player : MonoBehaviour
{
    ...
    public bool isPlayerinside_Item = false; // 오브젝트 구역에 들어갔는지 확인하는 변수
    public Collider2D currentObject = null; // 현재 충돌한 오브젝트를 저장할 변수
    public Collider2D currentObject_Weapon = null; // 현재 충돌한 오브젝트를 저장할 변수
    public Collider2D currentObject_Item = null; // 현재 충돌한 오브젝트를 저장할 변수
    ...
    void OnInteract(InputValue value)
    {
        ...
        {
            // ItemDrop 스크립트가 있는지 확인하고 Drop() 호출
            ItemDrop itemDrop = currentObject.GetComponent<ItemDrop>();
            if (itemDrop != null)
            {
                itemDrop.Drop();  // 아이템 드롭 실행
            }
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            Debug.Log("상호작용 가능한 오브젝트와 트리거가 발생했습니다.");
        ...
        {
            Debug.Log("상호작용 가능한 오브젝트와 트리거가 발생했습니다2.");
            isPlayerinside_Item = true;       // 오브젝트 구역 확인
            currentObject_Item = collision;   // 현재 오브젝트에 저장
        }
    }
    ...
    public void OnTriggerExit2D(Collider2D collision)
    {
        ...
        {
            Debug.Log("무기 구역에서 나갔습니다.");
            isPlayerinside_Weapon = false;
            currentObject_Weapon = null;
        }
        else if (collision.CompareTag("Item"))
        {
            Debug.Log("아이템 구역에서 나갔습니다");
            isPlayerinside_Item = false;       // 오브젝트 구역 확인
            currentObject_Item = collision;   // 현재 오브젝트에 저장
        }
    }
